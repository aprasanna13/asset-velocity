...
## 📁 Comprehensive Project Structure & Subdirectories

- **`src/` (Source Directory)**: Contains the full frontend React and TypeScript source code.
  - **`components/`**: Global UI components (e.g., Authentication Gateway overlay, sidebars, and operational badges).
  - **`features/`**: Functional operational zones including `command-center`, `maintenance`, `scada-triage`, and network `topology`.
  - **`hooks/` & `services/`**: Custom React hooks for data caching and the integrated API client layer (`api.ts`).
- **`backend/`**: Stores the persistent database files, specifically the SQLite database and its WAL concurrency logs (`database.sqlite`, `database.sqlite-shm`, `database.sqlite-wal`).
- **`dist/`**: Contains the production-ready compiled and minified static assets generated by Vite.
- **`server.js`**: The Express backend and static file server.
- **`database.js`**: SQLite initialization and schema management script.
- **`Dockerfile`**: Optimized multi-stage container configuration for Google Cloud Run deployments.
